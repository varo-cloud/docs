...
```bash cURL
curl https://inference.varo.cloud/v1/generations/GENERATION_ID \
  -H "Authorization: Bearer YOUR_API_KEY"
```

```javascript JavaScript (fetch)
const response = await fetch(
  `https://inference.varo.cloud/v1/generations/${generationId}`,
  {
    method: 'GET',
    headers: {
...
API keys issued by Varo Cloud do not expire automatically. A key remains valid indefinitely until you explicitly delete it from the **API Keys** page in your dashboard. Deletion takes effect immediately — any in-flight request using a deleted key will be rejected.
...
| `403 Forbidden` | The API key is valid and recognized, but it does not have permission to perform the requested action. |
